...
/**
 * Same as OSCPortIn but not a daemon thread so the JVM doesn't exit.
 * Not a clean solution but good enough for now.
 */
public class OSCPortInNotDamon extends OSCPort implements Runnable {
	 ...
	 */
	public OSCPortInNotDamon(DatagramSocket socket) {
	 ...
	 */
	public OSCPortInNotDamon(int port) throws SocketException {
	 ...
	 *   or there is already a socket listening on it
	 */
	public OSCPortInNotDamon(int port, Charset charset) throws SocketException {
		this(port);
